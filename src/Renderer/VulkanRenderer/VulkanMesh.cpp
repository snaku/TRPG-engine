...
#include <Renderer/VulkanRenderer/pushConstant.hpp>
...
{
    Push pushData{};
    pushData.model = pushConstantModel_;
    pushData.lightDir = pushConstantLightDir_;

    vkCmdPushConstants(cmd, layout, stageFlags, offset, size, &pushData);
}

void VulkanMesh::updatePushConstantData(const glm::mat4& model, const glm::vec3& lightDir)
{
    pushConstantModel_ = model;
    pushConstantLightDir_ = lightDir;
}
